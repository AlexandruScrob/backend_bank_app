...
MAXIMUM_SETTLMENT_DURATION = 60.0 * 60 * 3  # 3 hours

# Mean time to complete a transaction
MEAN_SETTLEMENT_DURATION = uniform(0.3, 1) * MAXIMUM_SETTLMENT_DURATION
print(
    "Mean settlement duration: {:.2f} hours".format(MEAN_SETTLEMENT_DURATION / 3600.0)
)

# How much faster time passes, for testing
TIME_FACTOR = 60.0 * 30  # 1 second simulation time = 30 minutes real time
        ...
        if i % 10 == 0:
            print(
                "Events: {} ({})".format(
                    len(events),
                    next(
                        (i for i in range(len(events)) if events[i] is None),
                        len(events),
                    ),
                )
            )
...
def api_function(f):
    ...
    def wrapper(*args, **kw):
        ...
        if time.time() < FAIL_UNTIL:
            # Oops
            FAIL_UNTIL += 60.0 * 60 / TIME_FACTOR
            print(
                "Failing for one more hour, {:.3f} seconds left".format(
        ...
        if rand.random() < FAIL_PROBABILITY:
            # Fail for 1-24 hours
            FAIL_UNTIL = time.time() + (60.0 * 60 * uniform(1, 24)) / TIME_FACTOR
